...
class ConfigLoadingTests(unittest.TestCase):
    ...
    def test_defaults_resolve_under_project_root(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(14, config.service.task_hourly_limits["mal_list_refresh"])
            self.assertEqual(48, config.service.task_hourly_limits["sync_apply"])
            self.assertEqual(40, config.service.task_hourly_limits["recommend_full_harvest"])
            ...
            self.assertEqual(10, config.service.task_projected_request_counts["mal_list_refresh"])
            ...
            self.assertEqual(5, config.service.task_execute_limits["recommend_provider_eligibility_candidates"])
            self.assertEqual(1, config.service.task_execute_limits["recommend_provider_eligibility_queries_per_candidate"])
            self.assertEqual(0, config.service.task_execute_limits["sync_apply"])
            self.assertEqual(30, config.service.task_projected_request_counts["recommend_full_harvest"])
            self.assertEqual(3, config.service.task_execute_limits["recommend_full_harvest"])
            self.assertEqual(10, config.service.task_execute_limits["recommend_full_harvest_pages"])
            self.assertEqual(10, config.service.task_execute_limits["mal_list_refresh_pages"])
    ...
    def test_settings_example_standard_toml_features_parse_and_preserve_precedence(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(40, parsed["service"]["task_hourly_limits"]["recommend_full_harvest"])
            self.assertEqual(30, parsed["service"]["task_projected_request_counts"]["recommend_full_harvest"])
            self.assertEqual(3, parsed["service"]["task_execute_limits"]["recommend_full_harvest"])
            self.assertEqual(10, parsed["service"]["task_execute_limits"]["recommend_full_harvest_pages"])
            self.assertEqual("all", config.openclaw.recommendations_webhook_delivery_mode)
