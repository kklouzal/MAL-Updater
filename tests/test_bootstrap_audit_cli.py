...
class BootstrapAuditCliTests(unittest.TestCase):
    ...
    def test_bootstrap_audit_surfaces_health_recommended_provider_snapshot_refresh(self) -> None:
        runtime_root = self.project_root / ".MAL-Updater"
        for relative in ("config", "data", "cache", "state", "secrets", "state/health"):
            (runtime_root / relative).mkdir(parents=True, exist_ok=True)
        (runtime_root / "data" / "mal_updater.sqlite3").write_text("", encoding="utf-8")
                        ...
                        "recommended_commands": [
                            {
                                ...
                                "detail": "Refresh the Crunchyroll ingest so stale provider rows are updated before the next review/apply cycle",
                                "command_args": [
                                    "crunchyroll-fetch-snapshot",
                                    "--out",
                                    ".MAL-Updater/cache/live-crunchyroll-snapshot.json",
    ...
    def test_bootstrap_audit_summary_surfaces_health_recommended_provider_snapshot_refresh(self) -> None:
        runtime_root = self.project_root / ".MAL-Updater"
        for relative in ("config", "data", "cache", "state", "secrets", "state/health"):
            (runtime_root / relative).mkdir(parents=True, exist_ok=True)
        (runtime_root / "data" / "mal_updater.sqlite3").write_text("", encoding="utf-8")
            ...
            json.dumps(
                {
                    "maintenance": {
                        "recommended_commands": [
                            {
                                "reason_code": "refresh_ingested_snapshot",
                                "detail": "Refresh the HIDIVE ingest so stale provider rows are updated before the next review/apply cycle",
                                "command_args": [
                                    "provider-fetch-snapshot",
                                    "--provider",
                                    "hidive",
        ...
        self.assertEqual(0, exit_code)
        self.assertIn("provider_hidive_ready=True", stdout)
        self.assertIn("provider_hidive_operation_mode=ready-health-recommends-snapshot-refresh", stdout)
        self.assertIn(
            "provider_hidive_next_command=PYTHONPATH=src python3 -m mal_updater.cli provider-fetch-snapshot --provider hidive --out .MAL-Updater/cache/live-hidive-snapshot.json --ingest",
            stdout,
        )
        self.assertIn("provider_hidive_next_command_reason_code=refresh_ingested_snapshot", stdout)
        self.assertIn("provider_hidive_next_command_automation_safe=True", stdout)
        self.assertIn("provider_hidive_next_command_requires_auth_interaction=False", stdout)
        self.assertIn(
            "maintenance_recommended_auto_command=PYTHONPATH=src python3 -m mal_updater.cli provider-fetch-snapshot --provider hidive --out .MAL-Updater/cache/live-hidive-snapshot.json --ingest",
            stdout,
        )
