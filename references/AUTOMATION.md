...
- the slower complete public MAL userrecs cold lane (`recommend_full_harvest`), including its independent cadence, bounded source-title/page posture, stale horizon, and most recent seed/harvest/failure/edge counts when task JSON stdout is available; a configured zero source-title limit disables the daemon lane instead of becoming an unbounded crawl
...
- Network ownership is explicit: provider hot fetch hourly, MAL user-list refresh every 8h, recommendation metadata every 12h, complete public MAL userrecs cold harvest every 6h with tiny bounded batches and a 45-day horizon, and one daily provider-eligibility task per credentialed provider. The hourly recommendation maintenance pass is DB/local-only.
