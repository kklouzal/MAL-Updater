...
- Network ownership is explicit: provider hot fetch hourly, MAL user-list refresh every 8h, recommendation metadata every 12h, complete public MAL userrecs cold harvest hourly with one source title per run and a 45-day horizon, and one daily provider-eligibility task per credentialed provider. The hourly recommendation maintenance pass is DB/local-only.
