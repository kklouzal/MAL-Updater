...
- Network ownership is explicit: provider hot fetch hourly, MAL user-list refresh every 8h, recommendation metadata every 12h, complete public MAL userrecs cold harvest hourly with two source titles per run, a per-source per-run page budget, and a 45-day horizon, and one hourly provider-eligibility task per credentialed provider (4 candidates, 1 alias, 5 returned matches). The hourly recommendation maintenance pass is DB/local-only.
