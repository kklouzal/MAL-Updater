...
- Network ownership is explicit: provider hot fetch hourly, MAL user-list refresh every 8h, recommendation metadata every 12h, and one daily provider-eligibility task per credentialed provider. The hourly recommendation maintenance pass is DB/local-only.
