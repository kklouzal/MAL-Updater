...
- bounded unattended execution posture for `sync_apply` via `service.task_execute_limits`; a configured zero disables/skips that scheduler lane rather than becoming an unbounded write pass (manual CLI `--limit 0` retains its explicit full-scan meaning), plus automatic reset of stale projected-request/backoff state when that execution posture changes so old full-pass history does not keep poisoning the lane
...
- Network ownership is explicit: provider hot fetch hourly, MAL user-list refresh every 8h, recommendation metadata every 12h, complete public MAL userrecs cold harvest hourly with two source titles per run, a per-source per-run page budget, and a deterministic 120 +/-15-day horizon, and one hourly provider-eligibility task per credentialed provider (2 candidates, 1 alias, 5 returned matches). The hourly recommendation maintenance pass is DB/local-only. Periodic durable positive evidence is stale-while-revalidate for read/display use, while absence claims keep shorter cadences and unattended writes require strict current eligibility; explicitly enabled `sync_apply` also requires successful same-cycle fetches from every configured provider.
