...
- continue tightening daemon orchestration and request-budget behavior (projected per-run request budgeting now uses explicit lane overrides or short rolling observed request-delta history; next likely step is deciding whether different lanes need different smoothing windows or more conservative high-percentile baselines)
