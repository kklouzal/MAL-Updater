...
- service task state now persists per-task cadence metadata (`every_seconds`), next-due timing, budget provider labels, and active backoff countdowns so unattended operators can see when a lane should run next vs when it is intentionally cooling down
