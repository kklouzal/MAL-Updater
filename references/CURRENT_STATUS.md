...
- continue tightening daemon orchestration and request-budget behavior (projected per-run request budgeting now uses explicit lane overrides, fetch-mode-specific task overrides, or tunable learned observed-history windows/percentiles, bursty lanes now auto-fallback to a conservative learned p90 when no explicit percentile is configured, `mal_refresh` plus `sync_apply` and Crunchyroll/HIDIVE full-refresh now have shipped built-in task defaults, and budget-blocked overdue full refreshes now degrade gracefully to incremental fetches instead of stalling the lane entirely; next likely step is deciding whether any other lanes deserve similarly justified shipped projection defaults)
