...
- continue tightening daemon orchestration and request-budget behavior (projected per-run request budgeting now uses explicit lane overrides, fetch-mode-specific task overrides, or tunable learned observed-history windows/percentiles, bursty lanes now auto-fallback to a conservative learned p90 when no explicit percentile is configured, and `sync_apply` plus HIDIVE full-refresh now have shipped built-in task defaults; next likely step is deciding whether Crunchyroll full-refresh or other lanes deserve similarly justified shipped mode-specific defaults)
