...
Publication is generation-wide and atomic. It requires authenticated account identity revalidated before publication, unchanged query/logic identity, complete `data` + `paging.next` structure, full nonterminal pages with strict expected-offset cursor progression, explicit terminal state for every partition, no duplicate staged MAL IDs, current fingerprints/anchors for every staged page, and strong empty proof where applicable. Validation hashes the exact stored page/row JSON bytes plus all publication-relevant cursor, counter, partition, terminal, account-epoch, generation-revision, and one-use local identity-assertion state; any post-validation mutation rejects in the publication transaction. A per-account publication epoch/current-generation fence prevents an older/different page-size or query generation from reusing a later assertion or overwriting a newer publication. Pruning scope is derived from persisted validated partitions: only the canonical `all` query can prune globally, while a subset may prune only its exact MAL statuses and always preserves unknown/out-of-scope rows. Contract/auth failures are quarantined terminally and cannot be selected or ordinarily reclaimed until explicit safe reinitialization. Publication SQL failure rolls back LKG/staging changes and releases the lease for a recoverable retry. Inspect the additive JSON `traversal` object for queue classes, cursors, revisions/claims, request counts, retry/drift/quarantine state, validation, and empty proof. MAL has no immutable list snapshot token; repeated upstream churn can therefore delay publication indefinitely by design while LKG remains published.
...
Both commands preserve published last-known-good rows and all prior generation history. They only create a fresh fenced generation; they do not fetch list/userrecs pages or publish data. The MAL-list command performs one authenticated identity lookup and requires an exact match to the quarantined account/query authority.
