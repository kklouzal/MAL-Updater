...
Effective daemon ownership defaults are intentionally separated: provider hot/incremental fetch hourly; MAL token refresh hourly; bounded MAL user-list refresh every 8 hours (10 network attempts and a matching 10-request cold projection against a 14-request task-hour cap, with durable partial pagination and no pruning unless terminal completion is proven); recommendation metadata every 12 hours (3 seed rows plus 5 discovery targets); complete public MAL userrecs cold harvest hourly (3 source titles, 10 network attempts per source per run including final revalidation, resumable staging, deterministic 120 +/-15-day complete-refresh horizon); provider eligibility hourly in one credentialed-provider-specific lane (5 candidates, strict uncovered-first with refresh only when uncovered cannot fill capacity, 1 alias each, 5 returned matches); and DB/local recommendation snapshot/health hourly. The full-userrecs cold-start projection is 30 requests (3 x 10) against a 40-request task-hour cap, leaving it below the 0.8 warning gate while retaining the 120/hour global MAL budget, pacing, and learned-history gate. Unattended `sync_apply` ships disabled with execution limit `0`; an explicit positive config opt-in is required, and once enabled every configured provider fetch lane must succeed in that same scheduler cycle before applying, so an old provider cache cannot authorize writes merely because the apply task is due. Eligibility lanes and the full-userrecs lane use both their task budget and the provider-global MAL/provider budget. Crunchyroll cold/full refresh is weekly and capped at 10 history + 2 watchlist pages; HIDIVE full refresh remains manual because it has no chunk controls. Service configuration is process-start state: restart `mal-updater.service` after edits before trusting `service-status` as the effective-policy view.
...
```bash
cd <repo-root>
PYTHONPATH=src python3 -m mal_updater.cli mal-list-refresh --max-pages 10
PYTHONPATH=src python3 -m mal_updater.cli recommend-refresh-full-userrecs --limit 3 --max-pages 10
...
Atomicity guarantee: fetched pages/edges are staged in an open generation and remain non-authoritative until a complete recognized userrecs document, unambiguous validated pagination, a coherent terminal chain, and immediate revalidation of every staged page prove the exact staged revision. Publication then replaces the source's recommendation graph in one transaction. `--max-pages` counts every actual network attempt, including resume and all final revalidations; orchestrated calls disable inner retries. Exhaustion pauses rather than publishing, and zero returns before queue sync/claim/fairness or outcome mutation. Malformed/truncated HTML, any unsafe or conflicting next candidate, loops/drift, oversize bodies, and transient HTTP failures preserve previous edges.

Source scheduling is durable and strict uncovered-first. Capacity is filled by class in this order: **never started**, resumable incomplete generation, retry-due failure, then routine stale/expiry refresh. Fairness inside a class uses a durable collision-free monotonic selection sequence, not second-resolution timestamps. A source claim is an expiring CAS lease; token, live expiry, source/generation identity, and generation revision fence every orchestrated mutation and publication. Long crawls renew only at safe request boundaries. Retry generation/queue timing and class are committed together.

MAL exposes no immutable userrecs snapshot token. Every resumed generation and every same-run terminal generation therefore re-fetches and fingerprints every staged page before publication. This costs one request per staged page; insufficient validation budget pauses fail-closed. Duplicate targets, cursor fallback, unsafe/conflicting links, repeated/backward/skipped ranges, syntactic truncation, or unexpectedly shortened chains fail closed. Validation is persisted against the exact staged revision. Empty publication additionally requires a dedicated MAL empty-state container, recognized userrecs surface, complete document, zero rows/cursors, and final revalidation; ordinary paragraph text is diagnostic/manual-review only.
...
### Durable MAL user-list pagination runbook

`mal-list-refresh --status ... --max-pages N` retains its compatible option name but is a request-attempt budget over every MAL invocation, including authenticated `get_my_user`, continuation, every-page snapshot revalidation, and the pre-publication identity recheck. Orchestrated GETs disable inner retries and record each attempt before invocation. Requested statuses rotate fairly: never-started first, then resumable, retry-due, and routine refresh. An exhausted budget returns `partial`; rerunning with the same account/query resumes the persisted opaque cursor. `--max-pages 0` is read-only/no-network/no refresh-state mutation.

Publication is generation-wide and atomic. It requires authenticated account identity revalidated before publication, unchanged query/logic identity, complete `data` + `paging` structure, full nonterminal pages with strict expected-offset cursor progression, explicit terminal state for every partition, no duplicate staged MAL IDs, current fingerprints/anchors for every staged page, and strong empty proof where applicable. `paging.next` may be omitted only on a validated short terminal page (including an empty page), where it is canonicalized to `null`; a full page without `next`, malformed present `next`, or a short page with a non-null `next` fails closed. Validation hashes the exact stored page/row JSON bytes plus all publication-relevant cursor, counter, partition, terminal, account-epoch, generation-revision, and one-use local identity-assertion state; any post-validation mutation rejects in the publication transaction. A per-account publication epoch/current-generation fence prevents an older/different page-size or query generation from reusing a later assertion or overwriting a newer publication. Pruning scope is derived from persisted validated partitions: only the canonical `all` query can prune globally, while a subset may prune only its exact MAL statuses and always preserves unknown/out-of-scope rows. Contract/auth failures are quarantined terminally and cannot be selected or ordinarily reclaimed until explicit safe reinitialization. Publication SQL failure rolls back LKG/staging changes and releases the lease for a recoverable retry. Inspect the additive JSON `traversal` object for queue classes, cursors, revisions/claims, request counts, retry/drift/quarantine state, validation, and empty proof. MAL has no immutable list snapshot token; repeated upstream churn can therefore delay publication indefinitely by design while LKG remains published.
### Explicit MAL quarantine recovery

Quarantine is durable: ordinary list/userrecs runs and queue synchronization never clear it. After an operator verifies the exact account/query or public source provenance, use one of these audited, narrowly scoped maintenance commands:

```bash
mal-updater mal-list-reinitialize --status all --page-size 100 --reason "<review/ticket and operator rationale>"
mal-updater recommend-reinitialize-full-userrecs --source-mal-anime-id 1 --source-url https://myanimelist.net/anime/1/Cowboy_Bebop/userrecs --reason "<review/ticket and operator rationale>"
```

Both commands preserve published last-known-good rows and all prior generation history. They only create a fresh fenced generation; they do not fetch list/userrecs pages or publish data. The MAL-list command performs one authenticated identity lookup and requires an exact match to the quarantined account/query authority. A quarantined MAL-list v2 generation may be explicitly reinitialized into v3 after the operator verifies that the failure was the now-recognized short terminal page with omitted `paging.next`; ordinary refreshes never clear or reuse that quarantine automatically.
