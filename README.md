...
The image defaults to `ghcr.io/kklouzal/mal-updater:0.2.7`. For local development builds use:
...
MAL-Updater remains alpha software. Version 0.2.7 adds conservative recurring runtime housekeeping and threshold-gated SQLite compaction; version 0.2.6 restored graceful container shutdown while preserving the non-root application and other container hardening. Provider/MAL authentication, conservative MAL-write controls, and the control-plane hardening described above remain intact. Semver tags build multi-arch GHCR images, wheels/sdists, checksums, provenance/SBOM, a keyless Sigstore signature, and a curated release bundle. Back up before every upgrade. See `CHANGELOG.md` for release history and known limitations.
