...
`service-status` / `service-status --format summary` now surface persisted per-task cadence, decision timing, last-run start/finish/duration, next-due timing, budget-provider labels, budget backoff level (`warn` vs `critical`), active backoff countdowns, and whether a provider-specific cooldown floor extended the wait so unattended behavior is inspectable without reading raw state files.
