...
The image defaults to the pinned first container release (`ghcr.io/kklouzal/mal-updater:0.2.0`). For local development builds use:
...
MAL-Updater remains alpha software. Version 0.2.0 is the first supported container release; the legacy source/systemd path is advanced-use only. Semver tags build multi-arch GHCR images, wheels/sdists, checksums, provenance/SBOM, a keyless Sigstore signature, and a curated release bundle containing compose/env/docs. Patch releases are intended to be safe upgrades within the same minor line; backup before every upgrade. See `CHANGELOG.md` for release history and known limitations.
