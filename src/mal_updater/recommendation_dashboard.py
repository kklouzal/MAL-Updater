...
function recommendationControls(data){ const options = dashboardGenres(data).map(genre => `<option value=\"${esc(genre)}\"${genre === selectedGenre ? ' selected' : ''}>${esc(genre)}</option>`).join(''); return `<div class=\"recommendation-controls\"><label for=\"genre-filter\">Genre</label><select id=\"genre-filter\"><option value=\"\">All genres</option>${options}</select><label><input id=\"show-hidden-titles\" type=\"checkbox\"${showHiddenTitles ? ' checked' : ''}> Show hidden titles</label></div>`; }
function renderDashboard(data){
  ...
  const sectionHtml = Object.entries(data.recommendations?.sections || {}).map(([name, rows]) => {
    ...
    const genreRows = selectedGenre ? rows.filter(r => Array.isArray(r.genres) && r.genres.includes(selectedGenre)) : rows;
  ...
  document.getElementById('genre-filter')?.addEventListener('change', event => { selectedGenre = event.target.value; renderDashboard(data); });
