...
function genreAffinitySection(model){
  ...
  return `<section id="genre-affinity" class="card affinity-card" aria-labelledby="genre-affinity-title"><h2 id="genre-affinity-title">Genre affinity</h2><div class="affinity-layout"><svg class="affinity-chart" viewBox="0 0 400 400" role="img" aria-labelledby="genre-affinity-svg-title genre-affinity-svg-desc"><title id="genre-affinity-svg-title">Genre affinity radar chart</title><desc id="genre-affinity-svg-desc">Normalized affinity for ${esc(axes.map(axis => axis.genre).join(', '))}. Exact values follow the chart.</desc>${grid}${axisLines}<polygon class="affinity-area" points="${values}"/>${points}</svg><div><h3>Exact values</h3><ol class="affinity-details">${details}</ol><p class="muted">${esc(model.represented_title_count)} unique watched titles contributed genre metadata.</p></div></div></section>`;
}
